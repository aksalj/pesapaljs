...
exports.submitPayment = function(order, callback, mobile, card) {
    ...
    var params = order.payment.parameters;

    switch (order.payment.method.tag) {
        case "VISA": // Credit / Debit Card
        case "MASTERCARD":
            if(!card) throw new Error("No Card Info Supplied");
            params.FirstName = card.firstName;
            params.LastName = card.lastName;
            params.Email = card.email;
            params.Country = card.country;
            params.CountryCode = card.countryCode;
            params.PhoneNumber = card.phone;
            params.CreditCardNumber = card.number;
            params.ExpDateMonth = card.expirationMonth;
            params.ExpDateYear = card.expirationYear;
            params.Cvv2Number = card.cvv;
            break;
        default: // Mobile Money
            if(!mobile) throw new Error("No transaction code supplied");

            params.MobileNumber = mobile.phone;
            params.TransactionCode = mobile.code;

            break;

    }

    // HUH: Assume order.payment.method will always be POST
    request.post({url: order.payment.url, formData: params}, function (err, resp, body) {
        if(err) {
            ...
            var reference = null; // From redirect URI
