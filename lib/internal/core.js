 ...
 */
exports.submitPayment = function(order, callback, paymentData) {

    if(!order._scraper || !order.paymentMethod) throw new Error("Invalid/Unprepared order");
    if(!paymentData) throw new Error("No Card/Transaction Info  Supplied");

    var params = order._scraper.getPaymentParameters();

    switch (order.paymentMethod.tag) {
        case "VISA": // Credit / Debit Card
        case "MASTERCARD":

            // TODO: TEST ME TEST ME
            throw new Error ("Credit Card Submission Not Functional :(");
    ...
    var url = order._scraper.getPaymentURL();
    switch (order._scraper.getHTTPMethod().toUpperCase()) {
        case "POST":
            request.post({
                url: url,
                jar: cookieJar,
